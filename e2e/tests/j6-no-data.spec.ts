...
test.describe("J6 — No-data (⊘ not 0)", () => {
  test("owed goal → No data → required reason → ⊘ recorded, owe clears, gap not a zero", async ({
    ...
    await step("The reason set is the five locked teacher reasons", async () => {
      await expect(
    ...
    await step("Commit is blocked until a reason is chosen", async () => {
      await expect(
        record,
    ...
    await step("Pick the locked reason 'Testing' → Record enables", async () => {
    ...
    await step("Record: the row shows ⊘ (distinct from scored) and the owe clears", async () => {
      await record.click();
    ...
    await step("The ⊘ stores no numeric value and PAUSES the consistency run", async () => {
